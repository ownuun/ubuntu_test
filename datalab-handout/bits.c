...
음수는 시프트할 때 부호는 양수로 시프트를 하기 때문에 신경쓰지 않아도 되지만
버림 방향이 0에서 멀어지기 때문에 보정값이 필요하다.
먼저 s = x >> 31로 부호를 읽어온다. 양수면 0, 음수면 -1이 된다.
바이어스 b는 반올림의 효과를 위해 미리 소수점을 제거하는 값이다. 
음수인 경우에 미리 1을 더해주면 되기 때문에 
b를 설정해야 하는데 소수점에 영향을 주는 위치가 (1 << n) - 1 이므로 
(1 << n) + ~0으로 만들었다.
따라서 양수일때는 s&b는 0이 되어 보정값이 없기 때문에 x를 그대로 반환하게 된다.
음수일때는 s&b는  b가 되고 그 값을 x에 더해주면 반올림 효과를 준다.
이렇게 bs를 구했다면 bx를 오른쪽으로 n비트 시프트하면 2^n으로 나눈 것과 같게 된다.
이는 양수는 그냥 오른쪽 시프트한 것이고, 
음수는 반올림 효과를 주고 오른쪽를 하게 된다.
*/
//-----------------------------------------------------------------------------
/* 
...
/* 
일단 작업은 하위 n비트를 떼어서 위쪽으로 붙이고, 나머지는 시프트로 이동시킨다.
우선 l_m = (1 << n) + ~0(-1)로 하위 n비트만 1인 것을 만들고 
하위 n비트의 x정보를 r_b에 저장한다.

r_b를 왼쪽 끝으로 보내야 하니까 32-n만큼 이동해야 하는데,
s = (32 + (~n + 1)) & 31로 32-n을 계산하면서  32는 나오지 않게 했고, 
0~31 범위를 유지했다.
r_l = r_b << s로 잘라낸 비트를 위쪽으로 옮긴다.

왼쪽에 남은 비트들은 단순히 산술 시프트하면 부호비트가 채워지니 안 된다.
그래서 l_m = ~(((1 << 31) >> n) << 1)로 
일단 최상위비트는 1이고 나머지는 0인 것을 만들고, 
다시 오른쪽으로 n비트 시프트하고 왼쪽으로 한 비트 시프트하면 
최상위 비트는 0이면서 n
오른쪽 이동 뒤에 남겨야 할 비트만 1인 마스크를 만들었다.
r_p = (x >> n) & l_m로 산술 시프트 뒤에 남은 비트들을 깨끗하게 만들어준다.

마지막으로 r_l | r_p로 두 조각을 합치면 완성된다.
*/
//-----------------------------------------------------------------------------
/*
