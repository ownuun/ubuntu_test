...
int isLessOrEqual(int x, int y) {
  ...
  int diff = y + (~x + 1);
  int diff_sign = diff >> 31;
  int diff_nonneg = !(diff_sign & 1);ㄴㅇㄻㄴㄹ
  return (sign_diff & (x_sign & 1)) | ((!sign_diff) & diff_nonneg);
}
/* 
일단 부호가 다르면 x가 음수고 y가 양수인 상황만 따지면 된다.
그래서 x_sign과 y_sign을 비교해서 다르면 sign_diff가 1이 된다.
sign_diff가 1이면서 x_sign의 최하위 비트가 1이면(x < 0, y >= 0) 무조건 x <= y이다.
...
diff_nonneg = !(diff_sign & 1)을 만들어서 diff >= 0이면 1이 나오게 했다.

정리하면 sign_diff가 1이고 x가 음수면 바로 참,
sign_diff가 0이면 diff_nonneg로 y - x가 음수인지 판단해서 결론을 낸다.
*/
//-----------------------------------------------------------------------------
/* 
...
int rotateRight(int x, int n) {
  int low_mask = (1 << n) + ~0;
  int right_bits = x & low_mask;
  int shift = (32 + (~n + 1)) & 31;
  int rotated_low = right_bits << shift;
...
우선 low_mask = (1 << n) + ~0으로 하위 n비트만 1인 마스크를 만들고 right_bits에 저장한다.

right_bits를 왼쪽 끝으로 보내야 하니까 32-n만큼 이동해야 하는데,
shift = (32 + (~n + 1)) & 31로 32-n을 계산하면서 0~31 범위를 유지했다.
rotated_low = right_bits << shift로 잘라낸 비트를 위쪽으로 옮긴다.
...
int satMul3(int x) {
  ...
  int overflow_mask = overflow_bits >> 31;
  int sign = x >> 31;
  int tmin = 1 << 31;
  int tmax = ~tmin;
  int sat = (sign & tmin) | ((~sign) & tmax);
  return (overflow_mask & sat) | ((~overflow_mask) & tripled);
}
/* 
...
overflow_bits를 31만큼 오른쪽으로 밀면 overflow_mask가 부호 확장으로 가득 차게 된다.
...
마지막으로 overflow_mask가 -1이면 sat를, 0이면 tripled를 선택하도록
(overflow_mask & sat) | ((~overflow_mask) & tripled)로 결과를 조립했다.
*/
//-----------------------------------------------------------------------------
/* 
