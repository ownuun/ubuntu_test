...
my $tmpbase = $ENV{DATALAB_TMPDIR} || "/var/tmp";
my $tmpdir = "$tmpbase/datalab.$login.$$";
...
#
my $fallback_base = getcwd() . "/.datalab-tmp";
my $tmpdir_created = 0;
if (-w $tmpbase) {
    $tmpdir_created = 1 if mkdir($tmpdir, 0700);
}
if (!$tmpdir_created) {
    mkdir($fallback_base, 0700) unless -d $fallback_base;
    (-d $fallback_base && -w $fallback_base)
        or die "$0: Could not prepare fallback scratch base $fallback_base.\n";
    $tmpdir = "$fallback_base/datalab.$login.$$";
